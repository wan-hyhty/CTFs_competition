...
## Phân tích 
- Phần chương trình ta cần chú ý ở đây
...
- Thông thường ta sẽ sử dụng `%<số lượng kí tự>s` để giới hạn số byte nhập vào (ví dụ `%12s`)
- đoạn input này sẽ được lưu vào `input.text`
- Khi này struct có thể được chia như sau
...
00000000 input_info      struc ; (sizeof=0x110, align=0x8, copyof_217)
00000000                                         ; XREF: _Z12handle_inputv/r
00000000 text            db 256 dup(?)              //256
00000100 hCon            HighLevelConnoisseur ?     //8
00000108 lCon            LowLevelConnoisseur ?      //8
00000110 input_info      ends
```

- thì thằng `HighLevelConnoisseur` sẽ trả về flag cho ta, chỉ khi ta chọn `level = 3` nhưng đã có ta chỉ được phép nhập 1 và 2
- Bấy giờ ta sẽ ow `LowLevelConnoisseur` thành `HighLevelConnoisseur`
...
```c
    ...
    if (level == 1) {
      talk(input.lCon, choice);
```
- Khi này struct của ta trông như sau
![image](https://github.com/wan-hyhty/CTFs_competition/assets/111769169/00e10f1b-ad38-4f71-a062-8ddf65043a2a)
- Đến hàm 
```c
void talk(Connoisseur &con, int choice) {
  switch (choice) {
...
```
- Khi này nó truyền vào địa chỉ `low` nhưng đã bị ta ow thành `high` và thực thi tiếp class `high` là in flag
## Kết quả
```python
#!/usr/bin/python3
